...
{
    public partial class yemekDiger : Form
    {
        ...
        {
            conn.Open();
            string query = "select birim, Sum(net_yemek) as toplam from logo where kurum='Balpet' and yil='" + cbYil.SelectedItem.ToString() + "' group by birim ";
        ...
        {
            conn.Open();
            SqlCommand query = new SqlCommand("select Sum(net_yemek) as toplam from logo where kurum='Balpet' and yil='" + cbYil.SelectedItem.ToString() + "' ", conn);
