        ...
        private void dagilim()
        {
            try
            {
                if (conn.State == ConnectionState.Closed)
                {
                    ...
                    conn.Open();
                    string query = ("select birim, Sum(net_fazla_mesai) as toplam from logo where yil='" + cbYil.SelectedItem.ToString() + "' group by birim");
                    ...
                    decimal[] y = (from p in dt.AsEnumerable()
                                   ...
                                   select p.Field<decimal>("toplam")).ToArray();
