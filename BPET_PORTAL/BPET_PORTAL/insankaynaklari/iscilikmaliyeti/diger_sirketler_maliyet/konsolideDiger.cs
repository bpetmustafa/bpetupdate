...
namespace BPET_PORTAL.insankaynaklari.iscilikmaliyeti.diger_sirketler_maliyet
{
    public partial class konsolideDiger : Form
    {
        ...
        private void sec()
        {
            ...
            {
                conn.Open();
                string query = "select sum(genel_maliyet) from logo where ay='" + cbAy.SelectedItem.ToString() + "' and yil='" + cbYil.SelectedItem.ToString() + "'";
        ...
        {
            conn.Open();
            string query = "select ay,sum(genel_maliyet) as maliyet from logo where yil='" + cbYilY.SelectedItem.ToString() + "' group by ay";
            SqlCommand cmd = new SqlCommand(query, conn);
