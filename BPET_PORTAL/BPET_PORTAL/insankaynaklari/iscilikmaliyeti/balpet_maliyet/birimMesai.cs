...
{
    public partial class birimMesai : Form
    {
        ...
        private void dagilim()
        {
            try
            {
                if (conn.State == ConnectionState.Closed)
                {
                    ...
                    conn.Open();
                    string query = ("select birim, Sum(net_fazla_mesai) as toplam from logo where kurum='Balpet' and yil='"+cbYil.SelectedItem.ToString()+"' group by birim");
