...
{
    public partial class birimDagilim : Form
    {
        ...
        private void Doldur()
        {
            try
            {
                if (conn.State == ConnectionState.Closed)
                {
                    ...
                    conn.Open();
                    string query = ("select birim, sum(genel_maliyet) as birimler from logo where kurum='Balpet' and yil='"+cbYil.SelectedItem.ToString()+"' group by birim order by birim");
