...
{
    public partial class detay1 : Form
    {
        public detay1()
        {
            InitializeComponent();
        ...
        private void BirinciDoldur()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = ("select  grup, ((sum(karar_vermek_zor)*100)/(select  count(*) from memnuniyet)) as karar_vermek_zor from memnuniyet group by grup order by karar_vermek_zor  desc ");
                    ...
                    int[] y = (from p in dt.AsEnumerable()
                               ...
                               select p.Field<int>("karar_vermek_zor")).ToArray();
